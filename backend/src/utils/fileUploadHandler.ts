...
export async function uploadFiles(files:any[], contentType: string, videoName:string, filenames:any[]) {
    ...
    if (contentType === 'video') {
        ...
        publicIds.push(azureUploadResult?.blobName);
        ...
        return { 
            uploadedFiles: {
                video: {
                    ...
                    url: azureUploadResult?.cdnUrl,
                    public_id: azureUploadResult?.blobName,
                }
            },
            publicIds
        };
    ...
    if (contentType === 'file') {
        ...
        const uploads = await Promise.all(
          files.map(async (file: any, index: number) => {
            // const result = await uploadOnCloudinary(file.path, 'file');
            const result = await uploadToAzure(file.buffer, file.originalname, file.mimetype);
            // publicIds.push(result?.public_id);
            publicIds.push(result?.blobName);
            if (fs.existsSync(file.path)) fs.unlinkSync(file.path);
    ...
    if (contentType === 'text') {
        console.log("inside uplode Fle for content Type Text--->",files)
        if (!files || !Array.isArray(files) || files.length === 0) {
            return
        }
        ...
        const azureUploadResult = await uploadToAzure(File.buffer || fileBuffer, File.originalname, File.mimetype);
        // publicIds.push(uploadResult?.public_id);
        publicIds.push(azureUploadResult?.blobName);

        if (fs.existsSync(File.path)) fs.unlinkSync(File.path);
        console.log("uploadResult-->",azureUploadResult)
        console.log("returning from uplode file.....")
        return { 
            uploadedFiles: File.mimetype === 'image/*'  ? 
            {
                image: {
