...
import { replaceFileInAzure } from '@/utils/uploadToAzure';
...
export const createSubtopic = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (req.files && req.files.length !== 0) {
        ...
        } else if (validatedData.contentType === 'text' ) {
            console.log("--> contentType is text");
            if(uploadResult?.uploadedFiles.image){
                validatedData.image = uploadResult.uploadedFiles.image; // Optionally store full image object
            }
            if(uploadResult?.uploadedFiles.avatarvideofile){
                console.log("--> contentType is text with avatar video file :",uploadResult.uploadedFiles.avatarvideofile);
                validatedData.avatarvideofile = uploadResult.uploadedFiles.avatarvideofile; // Optionally store full image object
            }
      ...
      if (validatedData.contentType === 'text' && validatedData.text_content) {
        //commented bcz no need to create TTS for text 
      ...
      const subtopic = new SubtopicModel({
        ...
        avatarvideofile: validatedData.contentType === 'text' ? validatedData.avatarvideofile : undefined,
    ...
    try {
        await session.withTransaction(async () => {
            ...
            const hasNewContent = (contentType === 'video' && (videoUrl || uploadedFiles?.length)) ||
                                ...
                                (contentType === 'text' && text_content || avatarvideofile );
            ...
            if (isContentTypeChanged || hasNewContent) {
                ...
                switch (contentType || existingSubtopic.contentType) {
                    ...
                    case 'text':
                        console.log("--> contentType is text");
                        let audioUrl;
                        let avatarvideofileUrl;

                       
                        //need changes to save audio buffer
                        if (text_content) {
                            console.log("--> updateding text_content is provided");
                        ...
                        if (Array.isArray(req.files) && req.files[0] && req.files[0].fieldname === 'avatarvideofile') {

                            console.log("--> avatarvideofile is provided");
                            const fileBuffer = fs.readFileSync(req.files[0].path);

                           avatarvideofileUrl = await replaceFileInAzure(
                            existingSubtopic.avatarvideofile?.public_id,
                            fileBuffer,
                            req.files[0].originalname,
                            'video'
                         );

                         updateData.avatarvideofile = {
                             url: avatarvideofileUrl.cdnUrl,
                             public_id: avatarvideofileUrl.blobName,
                         };

                         console.log("--> avatarvideofileUrl", avatarvideofileUrl);
                        }
