...
//Create organsiaion without using transaction
// Create Organization
export const createOrganization = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Create organization directly (no session)
  const organization = await OrganizationModel.create({
    ...validatedData,
    admin: adminId,
  });

  // Update the admin user with the created organization ID
  await UserModel.findByIdAndUpdate(adminId, {
    $push: { created_orgs: organization._id },
  });

  // Send response
  return new ApiResponse(201, organization).send(res);
});
